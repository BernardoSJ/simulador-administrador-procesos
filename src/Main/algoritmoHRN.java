 ...
 * @author Bernardo SJ
 */
public class algoritmoHRN implements Runnable{
    ...
    public void run() {
        try {
            ...
            Logger.getLogger(algoritmoHRN.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    
    public void iniciarHRN() throws InterruptedException{
     Procesos p;
        while (!FrameMain.procesoslistos.isEmpty()) {
            int posPro=0;
            int prioridadMayor=FrameMain.procesoslistos.get(0).prioridad;
            for(int i=0;i<FrameMain.procesoslistos.size();i++){
                if(FrameMain.procesoslistos.get(i).prioridad>prioridadMayor){
                    prioridadMayor=FrameMain.procesoslistos.get(i).prioridad;
                    posPro=i;
                    break;
                }
            }
            p = FrameMain.procesoslistos.get(posPro);
            int pEnTab = 0;
            ...
            if (p.es == false) {
                ...
                FrameMain.procesoslistos.remove(posPro);
