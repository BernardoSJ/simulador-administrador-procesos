 ...
 * @author Bernardo SJ
 */
public class algoritmoSRT implements Runnable {
    ...
    public void run() {
        try {
            ...
            Logger.getLogger(algoritmoSRT.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    
    public void iniciarSRT() throws InterruptedException{
      Procesos p;
        while (!FrameMain.procesoslistos.isEmpty()) {
            int posPro=0;
            int rafagaMenor=FrameMain.procesoslistos.get(0).rafaga;
            for(int i=0;i<FrameMain.procesoslistos.size();i++){
                if(FrameMain.procesoslistos.get(i).rafaga<rafagaMenor){
                    rafagaMenor=FrameMain.procesoslistos.get(i).rafaga;
                    posPro=i;
                }
            }
